...
def test_column_transformations(sample_data: pd.DataFrame, config: ProjectConfig, spark_session: SparkSession) -> None:
    ...
    such as removing the arrival_date column and changing data types of Id and room_type_reserved.
    ...
    assert "arrival_date" not in processor.df.columns
...
def test_missing_value_handling(sample_data: pd.DataFrame, config: ProjectConfig, spark_session: SparkSession) -> None:
    ...
    required_car_parking_space, no_of_adults, and lead_time columns.
    ...
    assert processor.df["required_car_parking_space"].isna().sum() == 0
